...
export default class Order {
    ...
    */
    ...
    deleteThisOrder = async () => {
        //if (confirm(`Удалить заказ от ${this.#orderUser} на ${this.#orderDate}. Вы уверены?`)) {
            //if (confirm(`Удалить заказ. Вы уверены?`)) {
            if (confirm('Удалить заказ. Вы уверены?')) {
                await AppModel.deleteOrder({order_id: this.#orderID});
                document.location.reload();
            } else {
    ...
    renderOrder() {
        ...
        const parentElement = document.getElementById('small-boy');
        ...
        parentElement.appendChild(liOrderElement);
    }
