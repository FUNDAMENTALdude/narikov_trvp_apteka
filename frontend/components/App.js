    ...
    */
    ...
    showOrHideForm = (event) => {
        if (event.target.checked) {
            ...
            document.getElementsByClassName('form-toggler__indicator')[0].innerHTML='-';
        } else {
                ...
                switch(iname[0]) {
                    ...
                    case 'order-FIO':
                        if (elems[index].value.split(' ').length == 3) {
                            ...
                            alert('Некорректно введенное ФИО');
                            cycleBreak = true;
                        }
                        break;
                    case 'order-DATE':
                        ...
                        if (comparable < dt) {
                            alert('Выбранная дата доставки заказа некорректна.');
                            cycleBreak = true;
                        } else {
                    ...
                    case 'med-NAME':
                        ...
                        if (filterable.length > 0) {
                            alert(`${elems[index].value} уже есть в списке заказов.`);
                            cycleBreak = true;
                        } else {
                            filterable = this.#stock.filter((medicine) => medicine.medName == elems[index].value);
                            if (filterable.length > 0) {
                                if (filterable[0].medRecipeFlag) {
                                    if (recipe.includes(filterable[0].medName)) {
                                        ...
                                        alert(`Требуется рецепт для заказа ${elems[index].value}`);
                                        cycleBreak = true;
                                    }
                                } else {
                                    sendable.lists.push({
                    ...
                    case 'med-COUNT':
                        if (filterable[0].medCount >= Number(elems[index].value)) {
                            ...
                            if (!orderIsUnderChange) {
                                alert(`На складе не хватает "${filterable[0].medName}". В наличии ${filterable[0].medCount} шт.`);
                                cycleBreak = true;
                            }
                        }
                        if (orderIsUnderChange) {
    ...
    renderStockTable = () => {
        ...
        const beforeElement = document.querySelector('h1[class="page-header"]');
        ...
        headMedName.innerText = 'Наименование';
        ...
        beforeElement.parentElement.appendChild(tableElement);
    };
